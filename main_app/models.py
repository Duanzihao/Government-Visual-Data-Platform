...
def accumulate_two_information(use_need_information):
    ...
    conn = mysql_connect()
    cursor = conn.cursor()
    str_process = list(filter(None, use_need_information.split(",")))  # 此处必须加一个list，因为filter转化后要以list展示，否则报错error<filter object at 0x02C18A70>
    select = "SELECT * FROM govdata1 where {} = '{}' and {} = '{}'".format(str_process[0], str_process[1],str_process[2], str_process[3])
...
def intime_to_archive_num(use_need_information):
    ...
    #     return 0
    select = "SELECT * FROM govdata1 where INTIME_TO_ARCHIVE_NUM like '1'  and CREATE_TIME between '{}' and '{}'".format(start_time,end_time)
    exe_select = cursor.execute(select)
    # 这里返回的是一个整数
    return exe_select        
...
def regist_new_user(use_give_information):
    ...
    query = "insert into account (name, tel, cipher, identity) values ('{}', '{}', '{}', 'resident')".format(name, phonenum, password)
    cursor.execute(query)
    conn.commit()  
    return '注册成功'
...
# # 增加数据
# def insert_data(use_need_information):
#     conn = mysql_connect()
#     cursor = conn.cursor()
#     str_process = list(filter(None, use_need_information.split(
...
#     cursor.execute(query, (str_process[0], str_process[1], str_process[2], str_process[3], str_process[4],
#                            str_process[5], str_process[6], str_process[7], str_process[8], str_process[9],
#                            str_process[10], str_process[11], str_process[12], str_process[13], str_process[14],
#                            str_process[15], str_process[16], str_process[17], str_process[18], str_process[19],
#                            str_process[20], str_process[21], str_process[22], str_process[23],
#                            str_process[24], str_process[25], str_process[26]))
#     conn.commit()  # 只要是修改了表内容的操作，后面一定要提交，否则不起作用
#     cursor.close()
#     conn.close()

# # 删除数据
# def delete_database(use_need_information):
#     conn = mysql_connect()
#     cur = conn.cursor()
#     str_process = list(filter(None, use_need_information.split(
#         " ")))  # 此处必须加一个list，因为filter转化后要以list展示，否则报错error<filter object at 0x02C18A70>
...
def accumulate_one_information(use_need_information):
    ...
    str_process = list(filter(None, use_need_information.split(
        " ")))  # 此处必须加一个list，因为filter转化后要以list展示，否则报错error<filter object at 0x02C18A70>
    # print(str_process)
    select = "SELECT * FROM govdata1 where {} = '{}' ".format(str_process[0], str_process[1])
    exe_select = cursor.execute(select)
    return exe_select
...
def scan_all_data_in_infoback_database():
    ...
    for i in range(len(user_account_info)):
        ...
        for j in range(len(user_account_info[i])):
            if j == 2:
                return_user_account_info[i]['time'] = user_account_info[i][j].strftime("%Y-%m-%d %H:%M:%S")
            elif j == 0:
...
def government_insert_information(use_need_information):
    ...
    EVENT_TYPE_ID = {
        ...
        '专业事件采集': '695'}
    # SUB_TYPE_NAME：小类名称====================================================================================
    f = open('D:\\MyProject\\gov_3.1\\static\\dict_use.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    SUB_TYPE_ID = {}
    count = 0
    for line in lines:
    ...
    f.close()
    # COMMUNITY_ID：社区id=======================================================================================
    f = open('D:\\MyProject\\gov_3.1\\static\\community_id.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    COMMUNITY_ID = {}
    count = 0
    for line in lines:
    ...
    f.close()
    # STREET_NAME:街道名===========================================================================================
    f = open('D:\\MyProject\\gov_3.1\\static\\street_name.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    STREET_ID = {}
    count = 0
    for line in lines:
    ...
    f = open('D:\\MyProject\\gov_3.1\\static\\dispose_id.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    DISPOSE_UNIT_ID = {}
    count = 0
    for line in lines:
    ...
    f.close()
    # MAIN_TYPE_ID:主类名称============================================================================================================
    f = open('D:\\MyProject\\gov_3.1\\static\\main_type_id.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    MAIN_TYPE_ID = {}
    count = 0
    for line in lines:
    ...
    f = open('D:\\MyProject\\gov_3.1\\static\\event_property_id.txt', 'r', encoding='utf-8')
    lines = f.readlines()
    EVENT_PROPERTY_ID = {}
    count = 0
    for line in lines:
    ...
    cursor.execute(query, (count_insert_now,
                           ...
                           SUB_TYPE_ID[str_process[4]],  # 小类id：绑字典2,SUB_TYPE_ID
                           '坪山区',
...
def delete_has_been_used_info_in_infoback_database(use_need_information):
    ...
    str_process = list(filter(None, use_need_information.split("*")))
    # 下面能够提供的查询次序分别是街道，姓名，时间，时间是要有格式要求的
    query = "delete from  infoback where tel = '{}' and backtime = '{}' ".format(str_process[0], str_process[1],)
    cursor.execute(query)
    conn.commit()  # 只要是修改了表内容的操作，后面一定要提交，否则不起作用
    cursor.close()
    conn.close()
